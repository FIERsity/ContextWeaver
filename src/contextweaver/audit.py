...
def audit_project(root: Path, *, allow_mock: bool = False) -> dict[str, Any]:
    ...
    _check(checks, "project", project_path.exists() and source_path.exists(), {
        "project_json": str(project_path), "source_document": str(source_path),
    })
    ...
    _check(checks, "source_retention", bool(
        normalized.exists() and original is not None and original.exists()
    ), {
        "normalized": str(normalized), "original": str(original or ""),
        "format": source.source_format,
    })
    report_required = source.source_format in {"epub", "docx"}
    import_report = root / STATE / "import_report.json"
    _check(
        checks, "import_loss_report", import_report.exists() or not report_required,
    ...
    structure_ok = (
        bool(sections) and bool(segments) and bool(units)
        and len(section_ids) == len(sections) and len(segment_ids) == len(segments)
        and set(unit_segment_ids) == segment_ids and len(unit_segment_ids) == len(segment_ids)
        and all(item.section_id in section_ids for item in segments)
    )
    _check(checks, "stable_structure", structure_ok, {
        "sections": len(sections), "segments": len(segments), "units": len(units),
    })
    strategy_path = root / STATE / "translation_brief.json"
    strategy = read_json(strategy_path) if strategy_path.exists() else {}
    _check(checks, "translation_strategy", bool(
        strategy.get("principles") and strategy.get("target_style")
        and strategy.get("generated_by")
    ), {
        "path": str(strategy_path), "concept_rules": len(strategy.get("concept_rules", [])),
    })
    ...
    _check(checks, "section_summaries", nonempty_sections <= set(summaries), {
        "required": len(nonempty_sections), "current": len(nonempty_sections & set(summaries)),
        "records": len(summary_records),
    })
    ambiguities = read_jsonl(root / STATE / "ambiguities.jsonl", AmbiguityRecord)
    ambiguity_evidence_ok = all(
        item.section_id in section_ids and bool(item.evidence_segment_ids)
        and set(item.evidence_segment_ids) <= segment_ids for item in ambiguities
    )
    _check(checks, "ambiguity_evidence", ambiguity_evidence_ok, {
    ...
    _check(checks, "translation_coverage", translated_ids == segment_ids, {
        "translated": len(translated_ids), "required": len(segments),
    ...
    _check(checks, "non_mock_translation", non_mock or allow_mock, {
    ...
    _check(checks, "revision_integrity", _revision_integrity(records, segment_ids), {
        "records": len(records), "active": len(active),
    })
    ...
    _check(checks, "segment_review_coverage", active_reviewed == segment_ids, {
        "reviewed_active": len(active_reviewed), "required": len(segments),
    ...
    for section in sections:
        ...
        if fingerprint and any(
            item.scope_type == "section" and item.scope_id == section.id
            and fingerprint in {item.input_digest, item.output_digest}
            for item in scope_reviews
        ):
            section_current += 1
    _check(checks, "section_review_coverage", section_current == len(nonempty_sections), {
        "current": section_current, "required": len(nonempty_sections),
    })
    book_fingerprint = scope_fingerprint(root, segments) if segments else None
    book_current = bool(book_fingerprint and any(
        item.scope_type == "book"
        and book_fingerprint in {item.input_digest, item.output_digest}
        for item in scope_reviews
    ))
    _check(checks, "book_review", book_current, {
    ...
    if translated_ids == segment_ids and segments:
        blocking_errors = sum(
            item.severity == "error" for item in validate_project(root)
        )
    _check(checks, "deterministic_validation", blocking_errors == 0, {
        "blocking_errors": blocking_errors,
        "not_run_reason": "incomplete translation" if blocking_errors is None else "",
    })
    output_root = root / "output"
    artifacts = {
        name: output_root / name for name in (
    ...
    _check(checks, "complete_exports", artifact_exists, {
    ...
    _check(checks, "epub_readback", epub_readback, {
    ...
    _check(checks, "provenance", bool(
        provenance.get("source_title") and provenance.get("translator")
...
def _revision_integrity(records: list[TranslationRecord], segment_ids: set[str]) -> bool:
    ...
    for item in records:
        ...
        if (
            previous is None or previous.segment_id != item.segment_id
...
def _finish(
    root: Path, checks: list[dict[str, Any]], project_id: str = ""
) -> dict[str, Any]:
    report = {
