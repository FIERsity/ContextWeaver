...
def review_project(
    ...
    reviewed_record_ids = {
        record_id for review in reviews
    ...
    for unit in units:
        ...
        pending_segments = [
            segment for segment in packet.source_segments
            if segment.id in selected_or_all and segment.id in active
            and active[segment.id].id not in reviewed_record_ids
        ]
        skipped += sum(
            1 for segment in packet.source_segments
            if segment.id in selected_or_all and segment.id in active
        ...
        pending_packet = ContextPacket(
            unit.id, pending_segments, packet.previous_text, packet.next_text,
            packet.section_summary, packet.glossary, packet.entities, packet.reference_texts,
        ...
        if len(decisions) != len(inputs):
            raise RuntimeError(f"Reviewer returned {len(decisions)} decisions for {len(inputs)} translations")
        for segment, previous, decision in zip(pending_segments, inputs, decisions, strict=True):
            ...
            if decision.verdict == "revised":
                ...
                if not text:
                    raise RuntimeError(f"Reviewer requested revision without replacement for {segment.id}")
                ...
                output = TranslationRecord(
                    ...
                    "review-v1-agent-critic-reviser", datetime.now(timezone.utc).isoformat(),
                    hashlib.sha256(segment.text.encode()).hexdigest(), "completed", revision,
                    previous.id, "agent-review:" + (",".join(decision.categories) or "general"),
                )
            ...
            review = TranslationReview(
                ...
                segment.id, previous.id, output.id, adapter.name, adapter.model,
                datetime.now(timezone.utc).isoformat(), decision.verdict,
                list(decision.categories), decision.rationale, confidence,
            )
