...
6. Run `extract-knowledge`. Keep uncertain candidates `proposed`; proposed rows do not block automatic translation. Agent or human review may approve them later. For a discipline-specific sourced term, first run `terminology-research-plan PROJECT WORK.jsonl`. It emits bounded research tasks with source Segment evidence and source-scoped UNTERM/IATE starting points. Do not represent either source as universal or infer a Chinese term from an English-only entry. Return only strict sourced JSONL candidates through `terminology-import PROJECT CANDIDATES.jsonl`, where every row has the term, candidate translation, HTTPS source, short source excerpt, authority tier, confidence, and real source Segment IDs. Then run `terminology-adjudicate PROJECT` to create only proposals, or explicitly opt into `--approve-authoritative` for high-confidence standard/official sources. This is append-only evidence: it never overwrites a glossary row, and only `approved` rows enter a ContextPacket. After a correction, use `terminology-impact PROJECT --term TERM`, then selectively retranslate and review the listed scope. Read [references/review-files.md](references/review-files.md) when editing these files.
