...
10. Run `validate` after every translation, review, or knowledge change. Balanced numeric mode is the working default: source numbers missing from the translation are errors, while target-only anchors are warnings because a digit can be a legitimate explicit rendering of source evidence such as `May` to `5月`. Investigate warnings, then use `--numeric-mode strict` for final release preparation. Use repeatable `--segment` or `--section` for an incomplete pilot; scoped issues are stored separately and do not mark the whole book complete. Treat exit code 1 as review work, not a command failure. Do not silently waive errors.
