...
6. Run `extract-knowledge`. Keep uncertain candidates `proposed`; proposed rows do not block automatic translation. Agent or human review may approve them later. For a discipline-specific sourced term, use `terminology-import PROJECT CANDIDATES.jsonl`, where every strict JSONL row has the term, candidate translation, HTTPS source, short source excerpt, authority tier, confidence, and real source Segment IDs. Then run `terminology-adjudicate PROJECT` to create only proposals, or explicitly opt into `--approve-authoritative` for high-confidence standard/official sources. This is append-only evidence: it never overwrites a glossary row, and only `approved` rows enter a ContextPacket. Read [references/review-files.md](references/review-files.md) when editing these files.
