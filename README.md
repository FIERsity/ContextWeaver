...
When `--max-units` is omitted, `agent-batch` uses the transparent adaptive policy stored in `state/batch_strategy.json`: prefer one complete Section, with a safety ceiling of about 120,000 source characters or 100 TranslationUnits (normally no more than roughly 300 Segments), and never cross a Section boundary. A conservative online/API run can lower `--target-source-chars`; explicit `--max-units` remains available for calibration. The production default comes from the real-book pilot: a complete 215-Segment chapter contained 84,743 source characters and serialized to about 926 KB, which remained practical for sequential Agent processing while eliminating two unnecessary intermediate stops. Each ContextPacket remains independently bounded per TranslationUnit.
