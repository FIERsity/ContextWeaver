...
`zh-cn-academic` uses A4 margins, SimSun (宋体) regular for Chinese body text, Times New Roman for Latin text and Arabic numerals, SimHei (黑体) bold for Chinese headings, and a 1.55-line body grid; `compact` is a denser editable draft profile. Both DOCX and PDF outputs reflow Chinese text rather than copying source pagination. The renderer keeps ordinary Latin words and number runs intact at Chinese/Latin boundaries, while allowing long URLs to wrap safely.
...
Numeric validation defaults to `--numeric-mode balanced`: a source number missing from the translation remains an error, while a target-only numeric anchor is a review warning because it may be a legitimate explicit rendering of source evidence such as `May` to `5月`. Semantic anchors recognize common equivalent forms such as `half a million` / `50万` and `World War II` / `二战`. `--numeric-mode strict` blocks every unresolved mismatch, and the v1 release audit always uses strict mode.
