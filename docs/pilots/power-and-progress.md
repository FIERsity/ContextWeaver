...
The durable project currently has 1,236 of 3,025 active source-aligned translations and 1,236 matching current Segment reviews. This comprises the complete 33-Segment Prologue, all 37 Segments across the copyright/title, Contents, and Navigation/dedication Sections, all 122 Segments of Chapter 1, all 145 Segments of Chapter 2, all 151 Segments of Chapter 3, all 215 Segments of Chapter 4, all 159 Segments of Chapter 5, all 190 Segments of Chapter 6, and all 184 Segments of Chapter 7. Three image-only Segments use the deterministic structural passthrough without a model call. All eleven complete Sections have current coherence-review fingerprints. Chapters 1–7 are available as translated and bilingual Markdown/EPUB. Daily `relaxed` numeric validation keeps source-number omissions blocking while deferring target-only anchors; `balanced` retains them as review warnings, and the v1 publication audit continues to require strict validation.
...
A second benchmark tested Campaign scopes of approximately 298, 597, 1,193, and all 2,637 then-remaining Segments. They covered 2, 4, 7, and 14 Sections and about 122k, 247k, 542k, and 980k source characters. Fully expanding those ContextPackets would produce packages of 1.3 MB, 2.8 MB, 5.7 MB, and 11.4 MB, so the system stores a compact Campaign manifest and materializes only the next checkpoint. The first three model-window-derived checkpoints processed Chapters 5, 6, and 7 as complete 159-, 190-, and 184-Segment assignments. All had exact ID sets, complete current Segment reviews, no-revision Section coherence passes, four chapter artifacts, and readable EPUB archives. The Chapter 6 run extended semantic numeric parsing for compound English cardinals such as `seven hundred and fifty`, `fifty-five million`, and `three hundred million`. The Chapter 7 run then added a three-level numeric policy and source-evidence normalization for Chinese shared-century decades, `750多万`, written-word age ranges, and repeated-year compression. This removes false invented-number failures without weakening missing-source-number checks or the strict publication gate. The refreshed Campaign now targets the remaining 1,789 Segments in 11 checkpoints; the next complete 183-Segment Section is estimated at 259,317 tokens.
