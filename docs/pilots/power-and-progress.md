...
The durable project currently has 82 of 3,025 active source-aligned translations and 82 matching Segment reviews. This comprises the complete 33-Segment Prologue, all 37 Segments across the copyright/title, Contents, and Navigation/dedication Sections, and the first 12 Segments of Chapter 1. Three image-only Segments use the deterministic structural passthrough without a model call. All four complete Sections have current coherence-review fingerprints, and scoped deterministic validation reports zero issues for both the completed Sections and the new Chapter 1 batch.
