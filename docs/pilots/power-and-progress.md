...
The durable project currently has 703 of 3,025 active source-aligned translations and 703 matching Segment reviews. This comprises the complete 33-Segment Prologue, all 37 Segments across the copyright/title, Contents, and Navigation/dedication Sections, all 122 Segments of Chapter 1, all 145 Segments of Chapter 2, all 151 Segments of Chapter 3, and all 215 Segments of Chapter 4. Three image-only Segments use the deterministic structural passthrough without a model call. All eight complete Sections have current coherence-review fingerprints. Chapters 1–4 are available as translated and bilingual Markdown/EPUB; Chapter 4 balanced validation retains two non-blocking, source-backed numeric warnings for natural written-number renderings.
...
A second benchmark tested Campaign scopes of approximately 298, 597, 1,193, and all 2,637 then-remaining Segments. They covered 2, 4, 7, and 14 Sections and about 122k, 247k, 542k, and 980k source characters. Fully expanding those ContextPackets would produce packages of 1.3 MB, 2.8 MB, 5.7 MB, and 11.4 MB, so the system stores a compact Campaign manifest and materializes only the next checkpoint. Chapters 3 and 4 are now complete. Refreshing after Chapter 4 targets the remaining 2,322 Segments in 14 model-window-derived checkpoints rather than fixed 40k or 120k recovery chunks. Ten of the twelve remaining Sections fit into one checkpoint; the 250- and 685-Segment Sections split into two. The next complete 159-Segment Section is estimated at 223,603 total input-plus-output tokens against a 340,000-token usable budget.
