...
The durable project currently has 862 of 3,025 active source-aligned translations and 862 matching current Segment reviews. This comprises the complete 33-Segment Prologue, all 37 Segments across the copyright/title, Contents, and Navigation/dedication Sections, all 122 Segments of Chapter 1, all 145 Segments of Chapter 2, all 151 Segments of Chapter 3, all 215 Segments of Chapter 4, and all 159 Segments of Chapter 5. Three image-only Segments use the deterministic structural passthrough without a model call. All nine complete Sections have current coherence-review fingerprints. Chapters 1–5 are available as translated and bilingual Markdown/EPUB. Balanced numeric validation keeps source-number omissions blocking while retaining source-backed target reformulations as review warnings.
...
A second benchmark tested Campaign scopes of approximately 298, 597, 1,193, and all 2,637 then-remaining Segments. They covered 2, 4, 7, and 14 Sections and about 122k, 247k, 542k, and 980k source characters. Fully expanding those ContextPackets would produce packages of 1.3 MB, 2.8 MB, 5.7 MB, and 11.4 MB, so the system stores a compact Campaign manifest and materializes only the next checkpoint. The first model-window-derived checkpoint processed Chapter 5 as one complete 159-Segment assignment, estimated at 223,603 total input-plus-output tokens against a 340,000-token usable budget. Its ID set was exact, two validation-driven corrections were appended as revisions, all current records received Segment review, the Section coherence pass made no further revision, and all four chapter artifacts passed export and EPUB archive checks. The refreshed Campaign now targets the remaining 2,163 Segments in 13 checkpoints; the next complete 190-Segment Section is estimated at 261,720 tokens.
