...
The durable project currently has 222 of 3,025 active source-aligned translations and 222 matching Segment reviews. This comprises the complete 33-Segment Prologue, all 37 Segments across the copyright/title, Contents, and Navigation/dedication Sections, all 122 Segments of Chapter 1, and the first 30 Segments of Chapter 2. Three image-only Segments use the deterministic structural passthrough without a model call. All five complete Sections have current coherence-review fingerprints. Chapter 1 passes complete Section-scoped deterministic validation with zero issues and is available as translated and bilingual Markdown/EPUB. The first Chapter 2 batch also passes scoped validation with zero issues, has four readable partial artifacts, and resumes at the next pending TranslationUnit.
