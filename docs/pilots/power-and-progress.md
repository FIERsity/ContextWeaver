...
The durable project currently has 1,052 of 3,025 active source-aligned translations and 1,052 matching current Segment reviews. This comprises the complete 33-Segment Prologue, all 37 Segments across the copyright/title, Contents, and Navigation/dedication Sections, all 122 Segments of Chapter 1, all 145 Segments of Chapter 2, all 151 Segments of Chapter 3, all 215 Segments of Chapter 4, all 159 Segments of Chapter 5, and all 190 Segments of Chapter 6. Three image-only Segments use the deterministic structural passthrough without a model call. All ten complete Sections have current coherence-review fingerprints. Chapters 1–6 are available as translated and bilingual Markdown/EPUB. Balanced numeric validation keeps source-number omissions blocking while retaining source-backed target reformulations as review warnings.
...
A second benchmark tested Campaign scopes of approximately 298, 597, 1,193, and all 2,637 then-remaining Segments. They covered 2, 4, 7, and 14 Sections and about 122k, 247k, 542k, and 980k source characters. Fully expanding those ContextPackets would produce packages of 1.3 MB, 2.8 MB, 5.7 MB, and 11.4 MB, so the system stores a compact Campaign manifest and materializes only the next checkpoint. The first two model-window-derived checkpoints processed Chapters 5 and 6 as complete 159- and 190-Segment assignments. Both had exact ID sets, append-only validation corrections, complete current Segment reviews, no-revision Section coherence passes, four chapter artifacts, and readable EPUB archives. The Chapter 6 run also extended semantic numeric parsing for compound English cardinals such as `seven hundred and fifty`, `fifty-five million`, and `three hundred million`, avoiding false invented-number errors without weakening missing-source-number checks. The refreshed Campaign now targets the remaining 1,973 Segments in 12 checkpoints; the next complete 184-Segment Section is estimated at 257,315 tokens.
