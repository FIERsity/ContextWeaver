...
Numeric comparison canonicalizes information rather than requiring identical character forms. Explicit English calendar months adjacent to a year and Chinese numeric or written months map to the same `month:N` anchor, so `May 2023`, `2023年5月`, and `2023年五月` agree. The same conservative mechanism covers explicit decades, centuries, World War ordinals, and scaled quantities: `1980s` agrees with `20世纪80年代`, `World War II` with `第二次世界大战`, and `1.5 million` with `150万`. A modal `may` outside date syntax is not converted, and all unrelated year/day/value anchors remain independently required.
