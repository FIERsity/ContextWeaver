...
`academic-assets` is an explicit, resumable network step rather than an importer side effect. Its first provider is PLOS ONE: it fetches only figure resources referenced by JATS `fig` objects, writes them under `source/assets/`, and records URL, SHA-256, byte count, and status in `state/academic_assets.json`. `academic-pdf` is an optional ReportLab renderer that creates reflowed A4 source, translated, or bilingual drafts under `output/pdf/`; `academic-docx` emits matching editable documents under `output/doc/`. Target-facing exports require complete active translations and preserve figure/table proximity rather than source-page coordinates. JATS validation compares imported figure/table counts with normalized blocks and rejects a missing or hash-modified fetched figure.
...
External terminology evidence uses two supplementary append-only JSONL collections: `terminology_candidates.jsonl` retains each submitted candidate translation, HTTPS source, excerpt, authority tier, confidence, and source Segment evidence; `terminology_decisions.jsonl` records the selected candidate for a term and its rationale. The deterministic adjudicator ranks `standard`, `official`, `academic`, `publisher`, then `community`, with confidence as its second criterion. It never overwrites an existing glossary row. Its default output is a proposed glossary entry; an explicit `--approve-authoritative` policy may promote only high-confidence standard or official candidates. This keeps autonomous operation possible without treating arbitrary web search results as authoritative.

`terminology-research-plan` is the provider-neutral research adapter boundary. It derives a bounded JSONL work package from unresolved glossary rows and strategy concept rules, preserving only relevant paragraph or block-quote evidence (at most 800 source characters per evidence item) and the language pair. Tables, bibliography lists, footnotes, and other structural blocks cannot create terminology research tasks. It supplies source-scoped official starting points (currently UNTERM and IATE) rather than claiming they are universal dictionaries. A browsing Agent or a future provider adapter returns the existing strict candidate contract; the importer validates it before append. `terminology-impact` maps a corrected source term to stable Segment IDs, allowing the existing selective retranslation, review, and validation workflow to repair only affected prose.
...
Numeric validation has three deliberate operating levels. The default `relaxed` mode blocks source numbers that disappear or change but defers target-only anchors, which are often harmless consequences of dates, enumerations, unit conversion, or natural Chinese rendering. `balanced` records those additions as non-blocking review warnings. `strict` promotes every unresolved mismatch to an error and remains mandatory for the publication audit. Known semantic equivalents are normalized before all three comparisons; relaxing a working run does not waive the final provenance check.

`audit-repair-batch` turns only current strict-validation errors into bounded JSONL repair items. Each row preserves its current TranslationRecord ID, source/raw Markdown, failure evidence, and ContextPacket; it is read-only and refuses to overwrite an existing package unless `--force` is explicit. A repairing Agent responds through the existing two-field `translation-import` contract, producing an append-only revision that automatically makes Segment, Section, and book review fingerprints stale for re-review.

When a target-only numeric anchor is demonstrably a source-backed naturalization rather than a translation change, `audit-resolution-import` accepts a separate JSONL decision containing the Segment ID, current TranslationRecord ID, stable issue ID, rationale, and concise evidence. The pipeline downgrades only that exact current strict error to a resolved warning. Revisions automatically invalidate decisions by changing the active TranslationRecord ID; malformed, stale, or non-current issue decisions are rejected.
